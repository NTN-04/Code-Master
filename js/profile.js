...
function initSearch() {
    ...
    if (resourceSearchInput) {
        resourceSearchInput.addEventListener('input', function() {
            ...
            resourceItems.forEach(item => {
                const resourceTitle = item.querySelector('h3').textContent.toLowerCase();
                const resourceDesc = item.querySelector('p').textContent.toLowerCase();
                const shouldShow = resourceTitle.includes(searchTerm) || resourceDesc.includes(searchTerm);
...
function initSettingsForms() {
    ...
    const personalInfoForm = document.querySelector('#settings .settings-section:nth-child(1) form');
    if (personalInfoForm) {
    ...
    const passwordForm = document.querySelector('#settings .settings-section:nth-child(2) form');
    if (passwordForm) {
    ...
    const notificationForm = document.querySelector('#settings .settings-section:nth-child(3) form');
    if (notificationForm) {
...
if (removeBookmarkButtons) {
    removeBookmarkButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            if (resourceItem && confirm('Bạn có chắc muốn xóa dấu trang này không?')) {
